...
void ImageStorageUserHandler::loop(){
    ...
    while( true ){
        ...
        if ( ! packet.is_checksum_correct() ){
            messages_vector.pop();
            //refresh_keep_alive();
            printf(" ** Attempt to manipulate the integrity !!! Packet is dropped ** \n");
            continue;
        }
    ...
    Message __response("ok");
    Socket::UDPsend(sock_fd,&__response,destination,UPD_ENUM_COMMANDS::RECEIVE_SUCCESS);
