...
Client::Client()
{
    ...
    int addrlen = sizeof(destinationSA);
    if(getsockname(s, (struct sockaddr *)&destinationSA, &addrlen) == 0 &&
       destinationSA.sin_family == AF_INET &&
       addrlen == sizeof(destinationSA))
    {
        int local_port = ntohs(destinationSA.sin_port);
