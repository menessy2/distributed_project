...
status Socket::UDPsend_ACK_support_with_fd(int s, const char *filename, \
    ...
    unsigned int cached_remaining_packets = 0;
    ...
    std::set<unsigned int> latest_sent_packets;
    ...
    while ( ! packetsHandler->is_transmission_reached_to_end() )
    {
        do {
            ...
            if ( is_first_packet ){
                cached_remaining_packets = udp_packet.get_remaining_packets();
            ...
            if ( ( ++window_counter % cached_remaining_packets ) == 0 ){
                window_counter = 0;
                cached_remaining_packets = udp_packet.get_remaining_packets();
                latest_sent_packets = sent_packets;
                break;
            }
    ...
    sent_packets = latest_sent_packets;
...
status Socket::UDPsend_ACK_support_with_fd2(int s, const char *filename, \
                                    SocketAddress destination, \
                                    UPD_ENUM_COMMANDS cmd){
